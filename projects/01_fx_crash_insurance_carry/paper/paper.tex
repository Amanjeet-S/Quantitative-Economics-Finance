...
Reiswich and Wystup (2012) give the delta definitions, the non-monotonicity of the premium-adjusted call delta, the right-branch convention and the first-order condition for its maximum. J\"ackel (2020, Section 2, eq.~(20)) gives the single maximum in (b) through the inverse Mills ratio, the argument the notes write out. Reiswich (2010, Section 3.8) shows that a smile delta can fail to be monotone, and Fukasawa (2012, Lemma 2.1 and Propositions 2.1--2.2) shows by the same derivative calculation that $d_+$ and $d_-$ are strictly decreasing for every arbitrage-free smile, which Mingone (2023, Lemma 4.1) links to a unique strike for each delta. My own part of (c) is only the explicit slope bound, which can be checked on a fitted smile not assumed to be free of arbitrage. Across the calibrated one-month smiles of the primary sample, the condition in (c) holds over $\pm4$ at-the-money standard deviations at every month-end, and premium-adjusted deltas are monotone on the conventional branch everywhere.
...
The proof applies the implicit function theorem first to the strikes and then to the system, following the argument Reiswich (2010, Theorems 2 and 3) uses for the market-strangle calibration of a simpler smile. Le Floc'h and Kennedy (2014, Section 4, eqs.~(30)--(31)) invert the leading-order expansion of the Hagan formula (Hagan et al., 2002, eq.~(3.1a)) in closed form, recovering $(\alpha,\rho,\nu)$ from the at-the-money level, skew and curvature, which is an approximate, global counterpart of the local and exact statement here. The three equations are the conditions of Reiswich and Wystup (2012), to which Reiswich (2010, Section 3.2) calibrates SABR, and Henrard (2011) uses the implicit function theorem in the same way for any exactly determined calibration. In the smoothness lemma, the value of the Hagan formula's ratio at the money is known (Choi and Wu, 2021) and the integral representation is Hadamard's lemma (Milnor, 1963). What I claim as my own is the joint smoothness statement for $\beta=1$, including $\nu=0$, and the local well-posedness statement for SABR under conditions (i) to (iv), written for FX delta conventions. Because the result is local, the solver restarts from several points; condition (iv) is checked at every calibration through the solver's Jacobian, whose condition numbers confirm non-singularity everywhere. Under the smile-strangle reading the result holds without (iii).
...
R8 & Hagan volatility with $\beta=1$ is smooth; the calibration has a locally unique $C^1$ solution in the quotes under non-degeneracy conditions & P & Hagan et al.\ (2002); calibration conditions: Reiswich and Wystup (2012), Reiswich (2010, Section 3.2); implicit-function-theorem argument: Reiswich (2010, Theorems 2--3), Henrard (2011); lemma: Choi and Wu (2021), Milnor (1963); explicit leading-order inversion: Le Floc'h and Kennedy (2014); the joint smoothness and local well-posedness statements for SABR are mine \\
...
\item Bagnoli, M. and Bergstrom, T. (2005). Log-concave probability and its applications. \emph{Economic Theory} 26(2), 445--469. Consulted: published article, Section 2.1 and the discussion of the normal distribution.
...
\item Choi, J. and Wu, L. (2021). The equivalent constant-elasticity-of-variance (CEV) volatility of the stochastic-alpha-beta-rho (SABR) model. \emph{Journal of Economic Dynamics and Control} 128, 104143. Consulted: arXiv:1911.13123v5, eqs.~(4)--(5).
...
\item Fukasawa, M. (2012). The normalizing transformation of the implied volatility smile. \emph{Mathematical Finance} 22(4), 753--762. Consulted: arXiv:1008.5055v2, Section 2.
...
\item Henrard, M. (2011). Adjoint algorithmic differentiation: calibration and implicit function theorem. OpenGamma Quantitative Research, SSRN 1896329; published in the \emph{Journal of Computational Finance} (2014). Consulted: SSRN version of 1 November 2011, Sections 1--3.
...
\item Milnor, J. (1963). \emph{Morse Theory}. Annals of Mathematics Studies 51. Princeton University Press. Consulted: Section 2, Lemma 2.1.
\item Mingone, A. (2023). Smiles in delta. \emph{Quantitative Finance} 23(12), 1713--1728. Consulted: arXiv:2209.00406v1, Remark 2.4 and Lemma 4.1.
