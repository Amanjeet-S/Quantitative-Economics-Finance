...
\date{Version of 23 September 2026}
\author{}

\begin{document}
\maketitle

\noindent These notes prove the results listed in \texttt{theory/README.md}. Throughout, $\Phi$ and $\varphi$ are the standard normal distribution and density functions, $s=\sigma\sqrt{\tau}$, and for forward $F$ and strike $K$
\[
d_\pm(K)=\frac{\ln(F/K)\pm s^2/2}{s}.
\]
We use the identity $K\varphi(d_-)=F\varphi(d_+)$ repeatedly.
...
\begin{remark}
Jensen's inequality gives $\E^{\Q^d}(1/S_T)>1/\E^{\Q^d}S_T=1/F$. There is no contradiction with (d): the foreign investor's forward price is an expectation under $\Q^f$, not $\Q^d$ (Siegel's paradox). If $Z$ is only a strict local martingale, $\E Z_T<1$ and the construction fails (Carr, Fisher and Ruf, 2014).
\end{remark}

\section*{R7. Entropy decomposition of currency premia}

Fix $t$ and write $\E_t$ for $\E[\cdot\mid\mathcal F_t]$. For a positive random variable $M$ with $\E_t|\log M|<\infty$, the conditional entropy is $L_t(M)=\log\E_tM-\E_t\log M\ge0$. Let $M,M^*>0$ be $\mathcal F_{t+1}$-measurable discount factors in $L^2$ that price domestic and foreign payoffs in their own currencies, with $\E_tM=e^{-r_t}$, $\E_tM^*=e^{-r^*_t}$, $\E_t|\log M|,\E_t|\log M^*|<\infty$ and $MS_{t+1}/S_t\in L^2$, where $S$ is the domestic price of foreign currency. Write $m=\log M$, $\Delta s=\log(S_{t+1}/S_t)$, and $x_t=(f_t-s_t)-(r_t-r^*_t)$ for the covered-interest-parity basis.
...
\begin{remark}
The identity in (i) needs only that some pair of discount factors prices bonds and exchange rates consistently; completeness makes the pair unique. Complete markets imply cross-country discount-factor correlations that are hard to reconcile with consumption data (Brandt, Cochrane and Santa-Clara, 2006). Options on $S$ reveal the risk-neutral law of $\Delta s=m^*-m$, not the separate cumulants of $m$ and $m^*$, so they constrain but do not identify the split of $L_t(M)-L_t(M^*)$ across cumulant orders. Part (iii) is the entropy bound of Bansal and Lehmann (1997); hedged and unhedged carry returns bound the same $L_t(M)$.
\end{remark}
