...
\noindent These notes prove the results listed in \texttt{theory/README.md}. Results that are standard are marked as such and attributed; the proofs written here are my own unless a source is named. Throughout, $\Phi$ and $\varphi$ are the standard normal distribution and density functions, $s=\sigma\sqrt{\tau}$, and for forward $F$ and strike $K$
\[
...
The link between the second strike derivative of call prices and state prices is due to Breeden and Litzenberger (1978). I state and prove the characterisation below, with a measure-theoretic proof.
...
The premium-adjusted delta, its non-monotonicity in the strike and the market practice of taking the strike to the right of the delta maximum are described by Reiswich and Wystup (2012). The proposition below makes the shape precise; the proof is mine.
