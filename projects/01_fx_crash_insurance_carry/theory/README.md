...
The implicit-function-theorem argument follows Reiswich (2010, Theorems 2 and 3, Appendices A and B), who uses it for the volatility-strike function and for the market-strangle calibration of a simplified parabolic smile. Le Floc'h and Kennedy (2014, Section 4, eqs. (30)–(31)) invert the leading-order expansion of the Hagan formula in closed form, recovering (α, ρ, ν) from the ATM level, skew and curvature, which is an approximate, global counterpart of R8. The local and exact statement for the three SABR equations, the smoothness lemma for β = 1 and conditions (i) to (iv) are my own. The implementation is `src/qef/fx/smile.py`, in the variables (ln α, atanh ρ, ln ν).
