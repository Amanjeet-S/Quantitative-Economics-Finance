...
``"market"``  market strangle: the single volatility σ_ATM + BF, at the strikes
              ...
              must reproduce at those same strikes (Clark, 2011, ch. 3;
              Reiswich and Wystup, 2012).
...
def _unpack(x, F, tau, beta):
    return SabrSmile(F, tau, float(np.exp(x[0])), float(np.tanh(x[1])), float(np.exp(x[2])), beta)
...
def _residuals(x, q: SmileQuotes, bf_reading: str, beta: float, k_ms):
    ...
    return np.array([r_atm, r_rr, r_bf])


def calibrate_sabr(q: SmileQuotes, bf_reading: str = "market", beta: float = 1.0, x0=None, tol=1e-12) -> CalibrationResult:
    ...
    best = None
    for s in starts:
        sol = least_squares(_residuals, s, args=(q, bf_reading, beta, k_ms), method="lm", xtol=tol, ftol=tol, gtol=tol, max_nfev=2000)
